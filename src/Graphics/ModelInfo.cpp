...
void CreateRenderableFromModel(const Model* model, AnimationInstanceData* optionalAnim, const glm::mat4& transform, Renderable* renderable)
{
	...
	renderable->numCmds = numCommands;
	renderable->cmds = new PBRRenderCommand[numCommands];
	UpdateRenderableFromModel(model, optionalAnim, transform, renderable);

	// FILL ALL RENDER FLAGS EXCEPT TRANSPARENT
	for (uint32_t i = 0; i < renderable->numCmds; i++)
	{
		PBRRenderCommand* cmd = (PBRRenderCommand*)((uintptr_t)renderable->cmds + sizeof(PBRRenderCommand) * i);
...
void UpdateRenderableFromModel(const Model* model, AnimationInstanceData* optionalAnim, const glm::mat4& transform, Renderable* renderable)
{
	renderable->bound = { glm::vec3(FLT_MAX), glm::vec3(-FLT_MAX) };
	...
	for (uint32_t j = 0; j < model->numNodes; j++)
	{
		for (uint32_t k = 0; k < model->nodes[j]->mesh->numPrimitives; k++)
		{
			...
			if (!(model->nodes[j]->mesh->primitives[k].flags & PRIMITIVE_FLAG_UNSUPPORTED))
			{
				PBRRenderCommand* cmd = (PBRRenderCommand*)((uintptr_t)renderable->cmds + sizeof(PBRRenderCommand) * curCommandIdx);
				...
				renderable->bound.min = glm::min(renderable->bound.min, min);
				renderable->bound.max = glm::max(renderable->bound.max, max);
...
{
	if (renderable->cmds) delete[] (PBRRenderCommand*)renderable->cmds;
	renderable->cmds = nullptr;
	renderable->numCmds = 0;
}
