...
struct AudioPlaybackContext* AU_PlayAudio(struct AudioManager* manager, uint32_t audioFileIdx, float volume = 1.0f);
struct AudioPlaybackContext* AU_PlayAudioOnRepeat(struct AudioManager* manager, uint32_t audioFileIdx, float volume = 1.0f);
